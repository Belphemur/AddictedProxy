...
public interface IProviderEpisodeRefresher : IEnumService<DataSource>
{
    ...
    Task<Episode?> GetRefreshEpisodeAsync(ShowExternalId showExternalId, Season season, int episodeNumber, CancellationToken token);
    ...
    /// <param name="token">Cancellation token.</param>
    Task RefreshEpisodesAsync(ShowExternalId showExternalId, IEnumerable<Season> seasonsToRefresh, Func<int, Task> sendProgress, CancellationToken token);
}
